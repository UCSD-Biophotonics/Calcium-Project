...
% 
function Analyze_Calcium_Activity(directory,files,method,neurons,append,stim_pos,user_data,write, myFolder)
global f1 f2 f3 sub
...
%     suptitle('d/dt(\Delta F/ F_{min})');
    ...
    format = [];
    if(lower(analysis(anum))=='extract')
        if append == 0 || exist(myFolder+"\data\Output.csv",'file') == 0
            fid = fopen(myFolder+"\data\Output.csv",'w');
            C=[Input,Num,Extract];
            for i = 1:length(C)
                format = [format,'%s,'];
            end
            ...
            fid = fopen(myFolder+"\data\Output.csv",'a');
        end
    end
