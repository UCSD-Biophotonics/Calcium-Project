...
root = "Z:\Shockwave\November 10\";
filetype = '.xlsx';
final = table; %creates an empty matrix to be filled later with values
cutindex = 1;%CHANGE the cuts here
for i= 1:53
    filenum = "data"; 
    %
    filepath = strcat(root, filenum, filetype); %this creates/puts together the filename
    wholematrix = readmatrix(filepath); %reads the files
    cell_info = readtable(filepath,'Range','B1:BB1','ReadVariableNames',false);
    cell1 = table2cell(cell_info(:,i));
    cell = string(cell1);

    x = wholematrix(:,1); %creates an array of x values (times)
    y = wholematrix(:,i+1); %creates an array of y values (intensities)
    ...
    plateaurange_time = next_cuttime;%next_cuttime;
    ...
    final{i,1} = cell; %this serves as a label to identify the time with the csv file
    final{i,2} = peaktime;
    final{i,3} = peak;
    ...
    %output = table(final,'VariableNames',{'Cell','Peak_Time','Peak_Value','Plateau_Time','Plateau_Value','Half_Life','Rsquare','Num_Points'}); %writes the matrix containing all subtracted times to a single csv file containing all the times
    final.Properties.VariableNames = {'Cell','Peak_Time','Peak_Value','Plateau_Time','Plateau_Value','Half_Life','Rsquare','Num_Points'};
    output_filename = "Half-lifes";

    finalwrite = strcat(root,output_filename,filetype); %will write to the location where all the csv files are
    
    writetable(final,finalwrite);
